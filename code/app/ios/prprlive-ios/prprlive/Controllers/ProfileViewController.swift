...
ProfileDelegate {
    ...
    func logoutSuccess() {
        ...
        if let token = A0SimpleKeychain().stringForKey("token") {
            A0SimpleKeychain().deleteEntryForKey("token")
        }
    ...
    func descriptionWillStartEdit() {
        let doneBarButtonItem = UIBarButtonItem(barButtonSystemItem: UIBarButtonSystemItem.Done, target: self, action: "doneButtonPressed:")
