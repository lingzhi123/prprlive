...
//        connector.close(stream!.id, success: closeStreamSuccess, failure: closeStreamFailure)
        self.closeStreamSuccess()
    }
    
    @IBAction func rotateButtonPressed(sender: AnyObject) {
        camera!.switchCameraDirection()
        if (camera!.session!.cameraPosition == AVCaptureDevicePosition.Front) {
...
//        StreamConnector().close(stream!.id, success: closeStreamSuccess, failure: closeStreamFailure)
    ...
    override func viewDidLoad() {
        ...
        camera?.start((self.stream?.name)!, channelName: (self.stream?.channel)!)
//        camera!.addPreviewView(previewView)
        
//        messenger.connect(stream!.id)
//        messenger.receive(chatMessageReceived)
        
        NSNotificationCenter.defaultCenter().addObserver(self, selector: "forceClose:", name: "Close/Leave", object: nil)
        
        self.timer = NSTimer(timeInterval: kTimerInterval, target: self, selector: "ping:", userInfo: nil, repeats: true)
