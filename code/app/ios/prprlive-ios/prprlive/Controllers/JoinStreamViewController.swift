...
UIActionSheetDelegate, SelectFollowersDelegate, ReplayViewDelegate, UserSelecting, CollectionViewPullDelegate {
    ...
    @IBAction func closeButtonPressed(sender: AnyObject) {
        ...
        if streamPlayer != nil {
//            StreamConnector().leave(stream!.name, likes: likes, success: leaveSuccess, failure: leaveFailure)
            self.leaveSuccess()
        } else {
    ...
    func closeStream() {
        if streamPlayer != nil {
//            StreamConnector().leave(stream!.name, likes: likes, success: leaveWithAlertSuccess, failure: leaveFailure)
            self.leaveWithAlertSuccess()
        } else {
...
//                mes.disconnect(stream!.id)
    ...
    override func viewDidLoad() {
        ...
        NSNotificationCenter.defaultCenter().addObserver(self, selector: "forceLeave:", name: "Close/Leave", object: nil)
//        if !isRecent {
//            StreamConnector().join(stream!.id, success: joinSuccess, failure: joinFailure)
//        } else {
            streamPlayer = StreamPlayer(stream: stream!, isRecent: isRecent, view: previewView, indicator: activityIndicator)
